...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<script>alert('genial usuario registrado exitosamente.')</script>";
				$username = "";
				$email = "";
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
			...
			echo "<script>alert('Disculpe, el email ya existe.')</script>";
		}
		
	} else {
		echo "<script>alert('Lo siento, el correo electrónico ya existe.')</script>";
	}
}
